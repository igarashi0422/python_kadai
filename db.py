...
def insert_user(user_name, password):
    sql = 'INSERT INTO user_kadai VALUES(default, %s, %s, %s)'
...
def login(user_name, password):
    sql = 'SELECT hashed_password, salt FROM user_kadai WHERE name = %s'
    flg = False
...
def select_all_books():
    ...
    sql='SELECT title, author, isbn FROM kadai_book'
...
def insert_book(title, author, isbn):
    connection = get_connection()
    cursor = connection.cursor()
    sql='INSERT INTO kadai_book VALUES (default, %s, %s, %s)'
...
def delete_book(isbn):
    connection = get_connection()
    cursor = connection.cursor()
    sql='DELETE FROM kadai_book WHERE isbn = %s'
...
def search_book(title):
    connection = get_connection()
    cursor = connection.cursor()
    sql = 'SELECT title, author, isbn FROM kadai_book WHERE title LIKE %s'
...
def update_book(title, isbn):
    ...
    sql="update kadai_book set title = %s where isbn = %s"
